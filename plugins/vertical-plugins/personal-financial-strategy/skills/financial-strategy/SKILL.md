...
1. **Tax year on every figure.** Pull numbers from `references/2026-quick-reference.md` (and `references/states/washington.md` for state figures) — never from model memory. State the tax year inline. If a figure carries a VERIFY flag in the tables, the flag travels with it into the answer.
...
4. **State overlay.** For any question that turns on state law — estate, inheritance, capital-gains, income, long-term-care, marital-property, or local/metro taxes — resolve the user's state **and**, where it can matter, their locality (city, county, transit district), then load that state's module from `references/states/` if it exists — files are named by two-letter code, so Washington is `references/states/washington.md`. `references/states/_index.md` says which modules exist and which cells are unresearched. Locality matters independently of residence: metro and city taxes commonly turn on where work is physically performed, not where the household lives or where the employer sits. If **no module exists** for the resolved state: give the federal analysis, then emit the canonical unbuilt-state fallback **verbatim** from `references/guardrails.md`, plus one flag line per `references/states/_index.md` cell marked `Yes` for that state — cells marked `VERIFY` contribute no flag line, because an unresearched cell has no fact to flag. State-level rules are where generic national guidance fails a household; for the default WA lens (rule 8), WA changed both its capital-gains and estate statutes within the last 13 months.
...
8. **Default household lens** (confirm before results depend on it): married filing jointly, Washington resident, W-2 wages plus LLC self-employment income, minor children. Both employee-side and owner-side options are in scope by default. **Ask rather than assume** when the scenario involves relocation, income earned across a state line, or real property in another state — the default is a starting point, not a finding. **Never assume state for an estate-situs question:** the wrong state there is the wrong tax regime, not a rounding error.
9. **Cite the controlling authority — and the label travels with the citations.** When a conclusion depends on a legal rule, name the authority (IRC/U.S.C./C.F.R. section, RCW/WAC, or agency issuance) from `references/authorities.md` and follow that file's retrieval protocol: pull current text before citing when the session has web access. **If any citation was not re-verified this session, the canonical index-only label from `references/authorities.md` appears verbatim, placed per that file's rule — a single unlabeled index-only citation is a defect, regardless of answer length.** Never cite a section number that is neither in the map, nor in the skill's own reference tables (which carry their own source citations), nor freshly retrieved. The purpose is conversational leverage — a client who walks in with the right section numbers and the right questions gets a better hour from the professional.
...
1. Intake: filing status, ages, income sources (W-2, self-employment, investment), account balances by tax bucket (taxable / tax-deferred / Roth / HSA). Then **three separate state answers**, because one will not cover them: (a) **state of residence**; (b) **state(s) where income is sourced** — physical work location, not employer location; (c) **state(s) where real property sits**. Take each from the prompt where stated, default residence to WA per rule 8, and ask only where the scenario implies they diverge (relocation, cross-border work, out-of-state property) rather than confirming all three every time.
...
3. Run the interaction checks (rule 3) and the state overlay (rule 4).
...
| `references/states/_index.md` | State resolution table: which modules exist, which are UNBUILT, and which cells are unresearched; source of the `Yes`-cell flag lines for the unbuilt-state fallback | Any question that turns on state law, before answering |
| `references/states/washington.md` | WA capital-gains excise tax (rate mechanics and the post-deduction $1M base), WA estate tax (split-year 2026, no portability), community property, WA Cares, WA↔OR cross-state stubs | Any WA question on gains, estates, LTC, or marital property |
| `references/states/_template.md` | Module authoring contract: fixed section order, VERIFY and planned-reference conventions, required sub-state layer | Authoring or refreshing a state module only — never to answer a question |
